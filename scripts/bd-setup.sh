...
# cannot do here (nidus-1oq, found in nidus): bootstrap reads the tracked `sync.remote`, rejects
...
[ -f .beads/metadata.json ] || die ".beads/metadata.json missing — is this the adelie repo?"
...
# database is the divergent case (nidus-1oq, found in nidus) and needs a human, so say so rather
# than guessing.
