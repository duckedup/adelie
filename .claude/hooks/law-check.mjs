// PostToolUse gate for the laws a script can settle at edit time: new `unsafe` and a Miri
// ignore with no stated reason. Catching them here removes a whole /adelie review round-trip.

import { readFileSync, existsSync } from 'node:fs'
import { execFileSync } from 'node:child_process'
import { resolve, relative } from 'node:path'

import * as laws from '../skills/adelie/lib/laws.mjs'
...
// Only an error blocks; a warning rides along when something else already blocked.
const errors = findings.filter(f => f.severity === 'error')
if (!errors.length) process.exit(0)
for (const f of findings) console.error(`${f.file}:${f.line} — ${f.summary}\n    ${f.detail}`)
console.error('\nFix these before continuing (AGENTS.md, ci.yml; adelie-check laws is the same detector).')
process.exit(2)
