...
// so an agent fetches §7 instead of reading all of SPEC.md.
...
function load() {
  ...
  if (!existsSync(path)) {
    console.error(docFlag === null
      ? `spec: there is no SPEC.md in this checkout (${REPO}), so there is nothing to address.
It may live on another branch: \`git show <branch>:SPEC.md > /tmp/SPEC.md\`, then
\`spec --file /tmp/SPEC.md toc\`.`
      : `spec: no such file: ${doc}`)
    process.exit(2)
  }
...
function runToc(lines) {
  ...
  if (!hs.length) { console.log('(no ## headings)'); return 0 }
...
function runFind(lines, words) {
  ...
  const file = docFlag === null ? '' : ` --file ${doc}`
  for (const f of found.slice(0, 10)) {
    const n = f.hits.length
    console.log(`${f.ref}  ${f.title}  (${n} line${n === 1 ? '' : 's'}) — fetch: spec${file} ${f.ref.replace(/^[§#]/, '')}`)
...
const USAGE = `spec — fetch one section of a repo doc instead of reading the whole file.
  ...
  spec --file AGENTS.md toc   any markdown file (default: SPEC.md)
