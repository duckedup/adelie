...
// the selftest needs no peers, no clones and no network. Ticket ids are full bead ids
// (`adelie-vnn`), used verbatim as subjects.

import { branchNamesIssue } from './git.mjs'
...
export function issueFindings(peers, issues = {}, opts = {}) {
  ...
  for (const p of peers) {
    for (const n of p.queue || []) {
      ...
      if (!meta) {
        findings.push(err('fleet-issue-missing', key, 'no such issue', 'bd could not resolve it. Do not dispatch a ticket whose contents you would have to invent.'))
        continue
      }
...
// Tickets that share a file will be individually green and jointly broken (found in nidus).
...
// derived from beads, GitHub PRs and the worktree registry, so a cleared session rebuilds the
...
export function rehydrate(peers = [], issues = {}, list = [], branches = []) {
  ...
  for (const p of peers) {
    for (const n of p.queue || []) {
      ...
      const names = ref => branchNamesIssue(String(ref || '').replace(/^origin\//, ''), id)
...
export function formatRehydrate(rows) {
  ...
  const sorted = [...rows].sort((a, b) => order[a.state] - order[b.state] || a.issue.localeCompare(b.issue))
  const lines = sorted.map(r => {
    const bits = [r.pr ? `PR #${r.pr}` : null, r.branch, r.via ? `(bundled with ${r.via})` : null].filter(Boolean).join('  ')
    return `  ${r.state.padEnd(9)} ${r.issue.padEnd(14)} ${r.holder.padEnd(14)} ${bits}`
  })
