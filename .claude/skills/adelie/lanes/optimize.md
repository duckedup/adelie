...
Staff-level performance work: make adelie do the same thing faster or cheaper, and **prove it
...
same tie-breaking, same errors; a change to any of those is a design change and needs an issue, not
...
Off the table without an issue first: weakening the durability order (write segment, fsync it,
publish the manifest, fsync the directory; SPEC §5 and §6), an `allow(unsafe_code)` (SPEC §1), a
dependency that costs build time (D0004), a non-additive on-disk format change.
...
1. **Preflight, then measure. You measure, not an agent.** SKILL.md's preflight has already
   ...
   - the benchmark harness that covers the scope (SPEC §13: ClickBench and the OTel-shaped
     suite, once they exist), output kept in a `mktemp -d`. If no harness covers the scope yet,
     writing one is the first unit, and this lane stops until it exists;
   - `cargo build --timings` or `just deps` if the scope touches Cargo.toml or feature gates;
   - any recorded baselines, and what each harness actually measures.
...
2. **Sweep.** `Workflow({ scriptPath: ".claude/skills/adelie/sweep.workflow.js",
...
8. **Re-measure, alone, and compare.** Re-run exactly step 1's commands on the merged tree,
   ...
   - Correctness first, always. `adelie-check lanes` says which CI jobs cover the change
     (test, Miri, the e2e binary); **Ship** watches them. A faster wrong answer is a
...
9. **Review** — `lanes/review.md`, unchanged, with `issues` set to the bead from step 4. Its
   ...
   number where the harness keeps its baselines.
