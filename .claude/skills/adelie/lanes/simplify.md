...
Staff-level cleanup: find where adelie says the same thing twice, or says a simple thing in a
...
1. **Preflight, then the control run.** SKILL.md's preflight has already run. Before you touch
   anything, get the tree green and record it: `adelie-check lanes --paths <scope>` names the
...
5. **You write the blueprints** — `lanes/spec.md` step 3 governs the format exactly, including
   the root blueprint, the per-directory files
...
8. **Prove nothing moved.** Re-run step 1's lanes on the merged tree and diff the result
   ...
   - the callers the lean lane never compiles: `src/cli`, `src/server`, `src/mcp`, the e2e
