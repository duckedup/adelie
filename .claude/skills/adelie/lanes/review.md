...
1. **Deterministic first.** `adelie-check laws --pr <n>` (or `--base origin/main`, or bare). These are
...
3. **Fan out.** `Workflow({ scriptPath: ".claude/skills/adelie/review.workflow.js",
   ...
   | `api-contract` | the public Rust API, HTTP DTOs, the SQL surface, MCP tools |
   ...
   Pass `issues: ["adelie-<id>"]` whenever the change claims a ticket: `scope` drops out
...
7. **Do not re-verify locally.** Fixes ride to CI with everything else; `adelie-check lanes`
...
8. `--fix`: apply the findings to the working tree and stop there.
   ...
   (`https://github.com/duckedup/adelie/blob/<sha>/<path>#L10-L15` — a `$(git rev-parse)`
