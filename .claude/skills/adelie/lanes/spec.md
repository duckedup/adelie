...
1. Run the research workflow. It fans out four fixed lenses (modules, tests, laws, prior art)
   ...
   `Workflow({ scriptPath: ".claude/skills/adelie/spec.workflow.js", args: { id, ask } })`
...
2. **Falsify the criteria you inherited, before the gate.** Take each acceptance criterion the
   ...
   research above is what makes this answerable — nidus-g4h's (found in nidus) "assert the reopened store
...
4. **You** write the blueprints from the research and the answers — do not delegate this. The
   ...
   **Several tickets are ONE blueprint set and ONE PR, never one per ticket.** When the target
   names more than one issue, they share a branch, a root blueprint and a PR
   whose body carries a `Closes` line each. Splitting them is the expensive default: N review cycles over one tree, and N chances to leave a bead open. `<id>` is then the tickets
   joined by `-` (`BLUEPRINT-adelie-hzi-61d.md`). One root blueprint covers all of them: one
   ...
   - `BLUEPRINT-<id>.md` at the **repo root**: summary, the table of sub-blueprints, complete
     ...
     it, from `adelie-check lanes` — a coverage map for the reader, not commands to run.
   - Never name these `SPEC-*.md` — `SPEC.md` at the root is adelie's product spec.
