...
2. Read every sub-blueprint. For each, compute its lanes:
   `adelie-check lanes --paths <that dir's files> --json`.
3. Fan out:
   `Workflow({ scriptPath: ".claude/skills/adelie/implement.workflow.js",
...
5. **A bug fix owes a deliberate fails-without-fix check.** AGENTS.md requires the regression
...
6. **Check scope before you trust the merge.** A patch is cut with `git add -A`, so it carries
   ...
   **Do not run the verification lanes here. CI runs them.** `adelie-check lanes` still tells
