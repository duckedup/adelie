...
**AGENTS.md's "Parallel sessions work in git worktrees" paragraph is what authorises that,
...
survives it forever and accumulates across a fleet. `adelie-check fleet` reports the orphans
...
```bash
.claude/skills/adelie/bin/adelie-check fleet            # state + findings
.claude/skills/adelie/bin/adelie-check fleet --status   # just the derived state
```
...
Every dispatch message carries all of: the tickets and their PR grouping, the order, `/adelie
implement` then `/adelie review` on their own diff before opening the PR, the worktree path,
`bd update adelie-<id> --claim`, and AGENTS.md's shipping laws (audit every `Closes` line against
the diff, close the bead yourself on merge, no session links in commits or PR bodies). Name who else is working where and on what — a peer that knows the shape of the other
...
`Cargo.toml` version means the second to merge releases nothing (D0005). `adelie-check fleet`
detects it, but not colliding beats detecting. Gaps are harmless, so never recycle a version
freed by a cancelled ticket without re-checking the tree first.
