...
use super::reader::{IndexEntry, SegmentReader, check_header, load_entry, resolve_entries, trailer_err};
...
impl IdxWriter {
    ...
    pub fn build<B: AsRef<[u8]>>(seg: &SegmentReader<B>, requests: &[(usize, IndexKind)]) -> Result<Vec<u8>, FormatError> {
        for &(column, kind) in requests {
            let field = seg
                .fields()
                .get(column)
                .ok_or_else(|| FormatError::Usage(format!("index names unknown column {column}")))?;
            if !kind.applies_to(&field.ty) {
                return Err(FormatError::Usage(format!("index kind {kind:?} does not apply to column {}", field.name)));
            }
        }
        ...
        for &(column, kind) in requests {
            for row_group in 0..seg.row_groups().len() {
                ...
                let Some(blob) = index::build(kind, &col) else { continue };
                let crc = crc32c(&blob);
                let offset = pos;
...
/// An open, validated `.idx` file, bound to the `SegmentReader` it was opened against.
...
impl<B: AsRef<[u8]>> IdxReader<B> {
    ...
    pub fn open<S: AsRef<[u8]>>(name: impl Into<String>, bytes: B, seg: &SegmentReader<S>) -> Result<Self, FormatError> {
        let name = name.into();
        let buf = bytes.as_ref();
        let footer_range = read_trailer(buf, IDX_MAGIC, IDX_HEADER_LEN).map_err(|e| trailer_err(e, &name))?;
        check_header(buf, IDX_MAGIC, &name)?;

        let bound_crc = u32::from_le_bytes(buf[8..12].try_into().unwrap());
        if bound_crc != seg.footer_crc() {
            return Err(FormatError::IdxMismatch { segment: name });
        }
        ...
        let indexes = resolve_entries(raw, seg.fields(), seg.row_groups().len(), IDX_HEADER_LEN as u64..footer_start, &name)?;

        Ok(IdxReader { name, bytes, fields: seg.fields().to_vec(), indexes })
    }
