...
use crate::format::wire::{Cursor, Sink, unzigzag, zigzag};
...
mod tests {
    ...
    fn max_decode_rows_with_tiny_payload_is_err() {
        ...
        let mut c = Cursor::new(&buf);
        assert!(decode_delta_i64(&mut c, crate::format::MAX_DECODE_ROWS).is_err());
        let mut c = Cursor::new(&buf);
        assert!(decode_dod_i64(&mut c, crate::format::MAX_DECODE_ROWS).is_err());
    }
}
